---
title: 'What is the state our of concepts of state groupings?: Hierarchical Clustering
  on states using the 2017 American Community Survey'
output:
...
The conception of regions is strong in how we make sense of our elections. "That won't play well in the Midwest/Northeast/etc." is easy to imagine coming out of a political consultant's mouth. There is a strong basis for this intuition. Historical and economic factors like migration patterns or location of industries rarely are isolated to one state and typically spill over into surrounding states. But time doesn't stand still and neither does demography, although it typically moves slowly. To study how similar states are to each other today, I downloaded data containing info from the 2017 American Community Survey and used hierarchical clustering to group them. Because I had many variables, I used principal components analysis for dimensionality reduction. I also downloaded data for the outcome of the elections to determine if the  principal components were able to separate states by their 2016 voting behavior. After dimensionality reduction, I used hierarchical clustering with complete linkage to cluster states based on the first 5 principal components.
...
Even after removing the highly correlated variables there are still too many variables for clustering, which is where the curse of dimensionality reared its head. In 24-dimensional space similar data points can be very far apart. For that reason, I used Principal Components Analysis to reduce the number of dimensions for which clustering was performed on.
...
Next, you have 3 Trump states (Texas, Arizona, and Florida) and 3 Clinton states (New Mexico, California, and Nevada). Out of the 6 states, 5 are in the Southwest. All have large hispanic populations, but Florida has a larger proportion of its hispanic population from Venezuelans and Cubans. Hispanic voters in Texas and Florida have historically been comfortable with the Republican party, but his is something to watch going forward.
...
plot(hc.complete, labels = X_state$State, main='Dendrogram of Hierarchical Clusters using data from the 2017 ACS', xlab='', sub='',cex=0.7)
```

# **III. Conclusions**

There are several interesting findings from the dendrogram. North Carolina, Georgia, and Virginia are less close to rest of the South than Michigan, Ohio, Indiana, and Missouri using the first 5 principal components of this data set. Florida is closer to the Southwest than to the South. There is a high degree of geographical continuity within the clusters, suggesting geography still matters. 

In terms of voting behavior, about half of the clusters were completely homogeneous in terms of the 2016 electoral outcomes for each state, but about half were mixed. This suggests the data set is missing important information related to voting behavior in the 2016 election. Two possible variables that might help are the distribution of age and educational attainment for the state. Despite some of the interesting clusters from this exercise, they are not gospel. This data set does not contain important variables (like median age and educational attainment) that could change the makeup of the clusters. Also, the first 5 principal components explain just under 80% of the variation in the data set. This leaves 20% of variation on accounted for, which could be omitting important information that could affect the clustering analysis.